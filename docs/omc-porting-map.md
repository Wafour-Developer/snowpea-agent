...
| `AskUserQuestion` tool | `ask_user` tool + `question.request` / `question.respond` | Implemented. The argument schema is a superset of `AskUserQuestion` and of Hermes' `clarify`. Interactive turns surface the question on the originating surface as an arrow-key picker (TUI `QuestionPrompt`); unattended and scheduled runs broadcast `question.pending`, which the bound messenger channel renders as numbered options with one button each. `questions.timeoutSec` (default 600); expiry, like Esc, is reported to the tool as a non-answer. |
