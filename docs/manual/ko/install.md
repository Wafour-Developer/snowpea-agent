...
현재 운영체제와 CPU에 맞는 데스크톱 앱을 설치하고 실행하려면 다음 명령을 사용하세요.

```bash
snowpea desktop
```

이미 설치되어 있으면 앱만 실행합니다. 설치된 버전과 최신 버전을 비교하려면 `snowpea desktop --check`, 업그레이드하거나 다시 설치하려면 `snowpea desktop --reinstall`, 실행하지 않고 설치만 하려면 `snowpea desktop --install-only`를 사용하세요. 스크립트에서는 `--yes`를 추가하면 대화형 확인 없이 다운로드를 승인합니다.
