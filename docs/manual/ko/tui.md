...
`↑`/`↓`(또는 `j`/`k`)로 옮기고, `1`–`9`는 그 줄로 바로 답하고, `Enter`는 선택한 줄을 고릅니다. 여러 개를 고를 수 있는 질문이면 `Space`로 켜고 끄고 `Enter`로 한꺼번에 보냅니다. 마지막 줄 `기타 / Other…`는 아무도 제시하지 않은 답을 위한 한 줄 입력을 엽니다. `Esc`는 "고르지 않겠습니다"라는 답이고, 에이전트는 침묵을 동의로 읽는 대신 거절되었다는 사실을 전달받습니다.
...
메신저 바인딩에서는 같은 질문이 번호 붙은 본문과 선택지별 버튼(그리고 "기타" 버튼)으로 도착하고, `2`(다중 선택이면 `1,3`)라고 답장해도 답이 됩니다. 헤드리스 `snowpea -c`는 그릴 선택 목록이 없으므로 타임아웃을 기다리지 않고 즉시 거절하며 stderr에 그 사실을 적습니다.
