...
클라이언트는 프롬프트가 대기열에 들어갈 때 `turn.queued`를, 빠져나와 시작될 때 `turn.dequeued`(reason `started`)를 함께 받습니다. 터미널 UI는 아직 대기열 표시를 그리지 않습니다(pending). 그때까지 대기 중인 프롬프트는 자기 턴이 시작될 때까지 조용히 기다립니다.
