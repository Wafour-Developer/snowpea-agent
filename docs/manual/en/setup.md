...
|---|---|---|
| `tools.readBeforeWrite` | `true` | `patch` and `write_file` add a `note:` to their result when this session has not read the file in full; a file a sibling subagent wrote after the read is refused with an `owned_by` error. Creating a new file is never noted. Set it to `false` to drop the guard. |
| `tools.maxResultLines` | `400` | Past this many lines, a `shell`, `execute_code`, `grep`, `glob` or `list_dir` result keeps a head and a tail; the middle goes to `$SNOWPEA_HOME/cache/tool-output/` and the result carries a `read_file` pointer with the offset and limit that would read it back. |
