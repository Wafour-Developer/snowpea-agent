...
`/model <ref>` はセッションをそのモデル id か、名前の付いたプロファイルに切り替えます。`/model` だけを打つと、読んで打ち直す必要のある一覧を出す代わりに、選ぶための画面が開きます。
...
並ぶのはまず設定のモデルプロファイル — `models.profiles` と、どれが既定でどのエージェントがどれを使うかを言う `models.default`・`agents.models` — 次にベンダーのエンドポイントが報告したモデル、最後に、ほかのどこにも出てこなければ今使っているモデルです。`Enter` はカーソルのある行で `/model <ref>` を実行します。ステータス行の `Model:` は使っているモデルを示し、デーモンが言うときはそれがどこから来たかも添えます。
...
待ち行列が減っていくあいだ、UI はそれを見せます。作業インジケータの行に `⏳ N queued` が付き、待っているプロンプトは実行される順に番号を振られて、入力行の下に淡く並びます。
...
自分の番が始まった瞬間、そのプロンプトは一覧から消えます。`Esc` は待ち行列をまるごと空にし、プロンプトごとに1行ではなく `2 queued prompts dropped` と一度だけ知らせます。

### プロンプトを1つエージェントに渡す

`$名前 ` で始まる入力は、メインのセッションではなくそのエージェントに行きます。送る前に入力行がそう言います。
...
名前はデーモンのエージェント一覧から来ます。誰もその名前で答えないときも `(no such agent)` を付けてそのまま表示します。`Enter` を押したあとより、押す前に気づくほうがよいからです。
