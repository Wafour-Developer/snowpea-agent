...
SKILL.md (agentskills.io): YAML frontmatter `name`, `description`, optional `argument-hint`, `user-invocable` (default true), `allowed-tools`; body = instructions. A skill becomes a **command** `/<name>` whose `run` injects the body (with `$ARGUMENTS` substituted) as a system+user instruction into the session and starts a turn. `skills/skill_md.py` parses; `skills/hooks.py` runs hooks; `skills/marketplace.py` aggregates sources; `skills/registry_client.py` = the hosted registry client (v0.3, live): `HttpRegistryClient` talks to `https://registry.snowpea.ai/v1` (overridable via `--registry`/`SNOWPEA_REGISTRY_URL`/`settings.skills.registry.url`), contributing a fourth `snowpea-registry`-labelled source to `marketplace.search`; a source outage answers with nothing rather than failing the aggregate. `skill.install registry:<id>` downloads and unpacks that skill's zip. `snowpea skill publish <dir>` (validates `SKILL.md` frontmatter, zips, `POST`s with a bearer publisher token — `--token`/`SNOWPEA_REGISTRY_TOKEN`/`settings.skills.registry.token`) and `snowpea skill rate <id> <stars>` are CLI-only, direct to the registry, no RPC method added. See `docs/design/deviations/CORE-registry-client.md` and the registry's own `registry-contract.md` for the full wire contract.
