...
```
> **Note (named local vendors).** `PRESETS` still holds exactly those 11 ids, but it is no longer the whole set of vendors. Any key under `settings.providers` whose block carries `"preset": "local"` (alias `"openai-compatible"`) is a local-style vendor of its own: `presets.synthesize_local_preset()` builds a `VendorPreset` from the block (id = the key, `base_url`/`variant`/`label` from the block, `key_required=False`, `supports_parallel_tools=False`, `local_style=True`), and `preset_for(vendor, variant, config)` returns it. `VendorPreset.local_style` replaced every `vendor == "local"` test in the registry, model discovery and the context-window prober, so a server called `hon2` gets keyless auth, `/v1/models` discovery, the Ollama `/api/tags` fallback, placeholder-model resolution and live window probing exactly like the built-in one. Names match `^[a-z][a-z0-9_-]{0,31}$` and may not shadow a preset id. `provider.list` carries `preset` and `custom` per row, `provider.remove` deletes a vendor with its model profiles and agent assignments, and `providers.local` keeps working untouched (it is local-style implicitly, with no marker). Deviations: [`deviations/CORE-named-local-vendors.md`](deviations/CORE-named-local-vendors.md).
...
> **Note (reasoning effort).** One user-facing scale — `low | medium | high | max` — reaches every vendor, and `providers/effort.py` is the single place that maps it. Precedence, highest first: a call override, the session pin (`session.setEffort`, `/effort`, persisted on the session row), `agent.effortBy["<vendor>:<model>"]`, `agent.effortBy["<vendor>"]`, `providers.openai.reasoning_effort` (read-only legacy, OpenAI only), `agent.effort` (default `medium`). Mappings: OpenAI-compatible → `reasoning_effort` (`max`→`high`) for the reasoning families only, with a 400 on the parameter remembered per model and the call retried once without it; the Codex transport → `reasoning.effort` (`max`→`xhigh`); Anthropic → `thinking.budget_tokens` and Gemini → `thinkingConfig.thinkingBudget`, both on the ladder 2 048 / 8 192 / 32 768 / 65 536 clamped to 75 % of the call's `max_tokens`; local-style vendors only when their block sets `effort_param: true`; every other vendor is sent nothing. `thinking: "off"` still wins over every tier. `VendorPreset.supports_effort` says which vendors take it at all and `ProviderInfo.supportsEffort` reports it; the `model.changed` event carries the effective `effort` and `effortSource`. Deviations: [`deviations/CORE-effort.md`](deviations/CORE-effort.md).

> **Note (vision capability).** `content.py::supports_vision` is now only the *name* rung (aliased `vision_from_name`); the chain lives in `ProviderRegistry.vision_for()` and answers `True | False | None`. Order: `providers.<vendor>.vision` or `providers.<vendor>.models.<model>.vision` → the cached models.dev card (`modalities.input` contains `image`, read from disk only, never fetched on the request path) → `VISION_HINTS`/`VISION_VENDORS` → for a local-style vendor only, `None`, meaning "try once". On `None` the adapter sends the image parts; a 400/415/422 whose body names image/content/multimodal is read as a refusal, remembered per `(vendor, base_url, model)` in `<SNOWPEA_HOME>/cache/vision.json` for a week, logged once, and the same turn retried immediately with the text fallback so it still completes. Success remembers `True`. A hosted vendor with no hint stays text-only as before. `provider.models` carries `vision: {model: bool}` for the ids it knows, `snowpea provider add-local --vision/--no-vision` writes the override, and the wizard's model list badges what it knows. Deviations: [`deviations/CORE-vision.md`](deviations/CORE-vision.md).
...
- `setup/catalog.py`: `CatalogItem(id, label, tier: free|paid|subscription, key: "no key"|"key optional"|"key required"|"self-hosted", default: bool, description)`, catalogs for `search` (from `tools/search_providers` registry order), `browser`, `tools` (categories with default on/off: file/terminal/git/web/browser/delegate/schedule/memory/skills/todo/session-search/clarify/cron on; media(image/video/tts) on-but-inactive; vision, computer-use, x-search off), `gateway` (telegram/discord/slack, all off), and (CORE-multimodal) `stt`/`tts` — the speech-to-text and text-to-speech option lists the Audio screen renders (`auto`, backend-specific rows, `off`; `active` reflects what is actually installed/configured here). The RPC `setup.catalog` (M8, `server/settings_handlers.py`) hands the IDE first-run wizard all seven catalogs so it can offer the same choices as `snowpea setup`, deriving `stt`/`tts` `active` flags from `audio.capabilities`'s own detection (`stt_providers`/`tts.available_providers`) rather than re-implementing PATH/key checks — CORE-setup-catalog-audio (v0.1.x).
